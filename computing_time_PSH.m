...
%% === 'theta' parameter ===
theta2 = (1.35:-0.05:0.900)*1.95*10^4; 
...
for iter = 1:N
    ...
    % [lselo_lqa(:,iter),opt_theta2(iter)] = lselo_LQA(n,initial_beta(:,iter),Z,1e-5,Delta,Iota,R,W,theta2);   % % LSELO
    % [lselo_cd(:,iter),opt_theta3(iter)] = lselo_CD(n,initial_beta(:,iter),Z,1e-5,Delta,Iota,R,W,theta2,block_size,p_size);   % % LSELO_CD

    % [selo_ista(:,iter),opt_lambda_selo(iter)] = ista_SELO(n,initial_beta(:,iter),Z,1e-5,Delta,Iota,R,W,lambda_selo);    % % SELO 
    % MIC_ista(:,iter) = ista_MIC(n,initial_beta(:,iter),Z,1e-5,Delta,Iota,R,W,a);    % % MIC
...
% fprintf(fid, '\n\n' );
% fprintf(fid,'%9.4s\t', 'LSELO', 'SELO', 'MIC','MCP','SCAD','ALASSO','LASSO');
% fprintf(fid, '\n' );
...
%                 ista_MIC()
% ============================================================
function [opt_beta,opt_a] = ista_MIC(n,ini_beta,Z,r,status,Iota,R,W,a)
[~,p] = size(Z);
f2 = zeros(n,p);
beta = ini_beta;
n0 = sum(status);
